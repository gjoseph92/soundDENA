...
"""
Reader - knows how files are formatted on disk

Takes: path to file, or possiblly directory of many similar files
Yields: pandas DataFrame of those files' contents, properly parsed
"""
...
def nvspl(dirpath, interval= 1, onlyColumns= None, quiet= True):
    ...
    for i, filepath in enumerate(filepaths):
            if i % interval == 0:
                ...
                if not quiet: print("Read NVSPL {} of {}".format(i+1, len(filepaths)))

    site = pd.concat(dataframes)
...
def audability(dirpath):
    ## TODO: doc
    if type(dirpath) is str:
        dirpath = pathlib.Path(dirpath)

    dataframes = []
    for filepath in sorted(dirpath.glob("LA_*.txt")):
    ...
    if len(dataframes) == 0:
        raise IOError("No listening center files in {}".format(str(dirpath)))
...
#     def __init__(self, readerVersion, tableNamesOrder):
#         """
#         Create a reader for metrics files of a specific version.

#         readerVersion   : float
#         tableNamesOrder : list of str

#         tableNamesOrder describes both the order in which tables occur in the file, and the attributes by which they'll
#         accessed in the named tuple returned by reading a metrics file. The ith table in the file will be accessed as the ith name in this list.
#         These table names should remain standardized across versions, but their order (and presence) may change.

#         When multiple tables describe similar data, such as median hourly metrics in dBA vs dBT, these can be combined
#         in a pandas Panel. In this case, give the name like "panelName_type": use the same `panelName` for each table (i.e. "contour"),
#         but suffix each with the distinguishing type of each table (i.e. "_L90", "_Lnat", etc.). All table names containing
#         underscores will grouped into Panels by the `panelName` portion; the `type`s will become the items for that Panel.

#         Example: `["percentTimeAbove_dBA", "percentTimeAbove_dBT", "contour_L90", "contour_Lnat", "contour_L50", "contour_L05"]`
#         will result in two Panels: - `percentTimeAbove`, with items `dBA` and `dBT`
#                                    - `contour`, with items `L90`, `Lnat`, `L50`, and `L505`
#         """
#         self.readerVersion = readerVersion
#         self.tableNamesOrder = tableNamesOrder

#         ## Determine which DataFrames to merge into Panels, based on underscored table names
#         self.panels = collections.defaultdict(list)
#         for tableName in tableNamesOrder:
#             parts = tableName.split("_")
#             if len(parts) == 2:
#                 panelName, itemType = parts
#                 self.panels[ panelName ].append( (tableName, itemType) )

#         # Insert version-specific info into the reader's docstring
#         #!!! setting .__func__.__doc__ modifies docstring for *all* instances
#         #!!! but just .__doc__ is read-only. So for now, this appears impossible
#         #!!! (without messing with metaclasses)
#         # self._read.__func__.__doc__ = self._read.__doc__.format(version= readerVersion,
#         #                                                panelsToTables= "\n        ".join(panelName+": "+", ".join(tableName for tableName, _ in components) for panelName, components in self.panels.items()),
#         #                                                numTables= len(tableNamesOrder))
#         # self.__call__.__func__.__doc__ = self._read.__doc__
...
#             except ValueError:
#                 raise ValueError('Cannot parse metrics file version from "{}"'.format(versionLine))
#         else:
...
#         Returns an object (a named tuple, really) with attributes for each table in the file, as well as
#         `metadata`, which is a dict of the colon-seperated key-value pairs in the file's header.

#         Each attribute for a table has two attributes itself: `data` and `n`. `data` contains a
#         pandas DataFrame (or Panel) of that table's data. `n` contains a TimeDelta of the length of that dataset.

#         Some tables in the metrics file contain overlapping data, but with different units.
#         (For example, median hourly metrics in dBA vs dBT). These tables are combined into one pandas Panel.

#         Examples (where the object returned from this function is stored as `metrics`):

#         ```
#         >> metrics.noiseFreeInterval.data
#         -> a DataFrame of the SPLAT Noise Free Interval (sec) table

#         >> metrics.noiseFreeInterval.n
#         -> a TimeDelta of the number of days used to compute the noise free interval metric

#         >> metrics.hourlyMedian.data
#         -> a Panel of the Median Hourly Metrics tables for dBA and dBT

#         >> metrics.hourlyMedian.data.dBA
#         -> a DataFrame subselecting the dBA item from the Median Hourly Metrics panel
#         -> (essentially, just the original Median Hourly Metrics (dBA) table in the metrics file)

#         >> metrics.hourlyMedian.data.major_xs('Leq').loc[0:12]
#         -> median Leq values from the hours 0-12 (rows) for both dBA and dBT (columns)

#         >> metrics.frequency.n
#         -> the *sum* of days in Median Nighttime Frequency Metrics and Median Daytime Frequency Metrics
#         ```
...
#         A warning is raised if the n-values for tables being combined into a Panel don't match. In this case,
#         n will be NaT. (The only exception is the Median Daytime/Nighttime Frequency Metrics tables, whose
#         n-values are summed.)
#         ```
#         """
...
#         header, *tables = sections

#         ## Parse metrics file version and check for compatability
#         try:
#             versionLine, titleLine, *headerLines = header.split("\n")
#         except ValueError:
#             raise ValueError("Unrecognized header in metrics file")
            
#         version = metricsReader.parseVersionLine(versionLine)
...
#         if len(tables) != len(self.tableNamesOrder):
#             # raise TypeError("Expected {} tables, but {} found in file".format(len(tables), len(self.tableNamesOrder)))
#             warnings.warn("{}: Expected {} tables, but {} found in file".format(pathlib.Path(path).name, len(self.tableNamesOrder), len(tables)))

#         ## Parse each table into a DataFrame
#         data = {}
#         ns = {}
#         for table, name in zip(tables, self.tableNamesOrder):

#             title, *lines = table.split("\n")
...
#                 if unit == "hrs": n = pd.to_timedelta(amt, unit= "h")
...
#             df.index.name= None
#             df.columns = header[1:]
#             df = df.convert_objects(convert_dates= False, convert_numeric= "coerce", convert_timedeltas= False, copy= False)
...
#             # Ensure percentTimeAbove has the same columns names in both tables, instead of dBA and dBT
#             if name in ["percentTimeAbove_dBA", "percentTimeAbove_dBT"]:
#                 df.columns = df.columns.str.slice(stop= -1)
            
#             data[name] = df

#         ## Create panels
#         for panelName, components in self.panels.items():
#             try:
#                 dfs = { itemType: data[tableName] for tableName, itemType in components }
#             except KeyError:
#                 # A component table was not actually in the metrics file. Skip this one.
#                 warnings.warn("Missing table(s) for {}".format(panelName))
#                 continue

#             p = pd.Panel(dfs)
#             panelNs = [ ns[tableName] for tableName, _ in components ]
#             if panelName == "frequency":
#                 # Sum day and night ns
#                 n = np.sum(panelNs)
#             else:
#                 if not all(panelNs[0] == n for n in panelNs):
#                     warnings.warn("In {}: n values differ between {} tables ({})".format(path, panelName, {tableName: ns[tableName] for tableName, _ in components}))
#                     n = pd.to_timedelta("NaT")
#                 else:
#                     n = panelNs[0]
#             for tableName, _ in components:
#                 del data[tableName]
#                 del ns[tableName]
#             data[panelName] = p
#             ns[panelName] = n
...
#         try:
#             freq = data["frequency"]
#             freq["overall"] = splMean(freq.day, freq.night)
#         except KeyError:
#             pass

#         try:
#             ambient = data["ambient"].transpose('major_axis', 'items', 'minor_axis')
#             ambient["overall"] = splMean(ambient["Day"], ambient["Night"])
#             data["ambient"] = ambient.transpose('major_axis', 'items', 'minor_axis')
#         except KeyError:
#             pass

#         try:
#             pta = data["percentTimeAbove"].transpose('major_axis', 'items', 'minor_axis')
#             pta["overall"] = (pta["Day"] + pta["Night"]) / 2
#             data["percentTimeAbove"] = pta.transpose('major_axis', 'items', 'minor_axis')
#         except KeyError:
#             pass


#         metricsTuple = collections.namedtuple('Metrics', list(data.keys()) + ["metadata"] )
#         metricTuple = collections.namedtuple('Metric', ["data", "n"])

#         metricsTuple.__doc__ += '\n\nEach metric is stored as a named tupe, with attributes `data` and `n`.'
#         metricsTuple.__doc__ += '\nFor example, if this named tuple is named `metrics`:'
#         metricsTuple.__doc__ += '\n    `metrics.noiseFreeInterval.data` would return a pandas DataFrame'
#         metricsTuple.__doc__ += '\n    `metrics.noiseFreeInterval.n`    would return a pandas TimeDelta of the length of the dataset'
#         metricsTuple.__doc__ += '\n\nMetadata is available as a dict in `metrics.metadata`.'

#         metricTuple.__doc__   += '\n\n`data`: a pandas DataFrame or Panel'
#         metricTuple.__doc__   += '\n`n`     : a pandas TimeDelta of the length of this dataset'


#         m = metricsTuple(metadata= header, **{ metric: metricTuple(df, ns[metric]) for metric, df in data.items() })
#         return m
...
#         "ambient_dBA",
#         "ambient_dBT",
#         "percentTimeAbove_dBA",
#         "percentTimeAbove_dBT",
#         "contour_L90",
#         "contour_Lnat",
#         "contour_L50",
#         "contour_L05",
...
class metricsReader:
    ...
    def __init__(self, readerVersion, metricNames):
        ...
        self.titlesToMetricNamesAndTypes = { title: (metricName, tableType) for metricName, typesAndTitles in metricNames.items() for tableType, title in (typesAndTitles.items() if type(typesAndTitles) is dict else [(None, typesAndTitles)]) }
    ...
    def __call__(self, path):
        ...
        metrics = { metricName: pd.Panel4D(panelDict) for metricName, panelDict in metrics.items() }
        ...
        ns = { metricName: pd.DataFrame(nVals) for metricName, nVals in ns.items() }
        for metricName, n in ns.items():
        ...
        try:
            ...
            for season in freq.labels:
                freq.loc[season, "overall"] = metricsReader.splMean(freq.loc[season, "day"], freq.loc[season, "night"])
        except KeyError:
            pass
        ...
        try:
            ...
            for season in ambient.labels:
                ambient.loc[season, "overall"] = metricsReader.splMean(ambient.loc[season, "Day"], ambient.loc[season, "Night"])
...
metricsReaders = { version: metricsReader(version, metricNames) for version, metricNames in metricsVersions.items() }
...
def metrics(path):
    ...
    version = metricsReader.parseVersionLine(versionLine)
    try:
        return metricsReaders[version](path)
    except KeyError:
        raise TypeError("No metrics reader for version {}".format(version))

metrics.__doc__ = metricsReader.__call__.__doc__