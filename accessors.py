...
class NVSPL(accessor.Accessor):
    pathToData = paths.nvspl

    @staticmethod
    def parse(dirpath, interval= 1, onlyColumns= None, quiet= True):
        """
    ...
    # TODO:
    # @classmethod
    # def _filepath(class, dataDir, unit, site, year):
        # join NVSPL and processed_nvspl and maybe partial days

class SRCID(accessor.Accessor):
    pathToData = paths.spl / "SRCID_{unit}{site}.txt"
...
class LoudEvents(accessor.Accessor):
    pathToData = paths.spl / "LOUDEVENTS_{unit}{site}.txt"
...
class Audibility(accessor.Accessor):
    pathToData = paths.wav

    @staticmethod
    def parse(dirpath):
        ## TODO: doc
        ## TODO: use glob for pathToData
        if type(dirpath) is str:
            dirpath = pathlib.Path(dirpath)
...
class DailyPA(accessor.Accessor):
    pathToData = paths.spl / "DAILYPA_{unit}{site}.txt"
...
class Metrics(accessor.Accessor):
    pathToData = paths.spl / "METRICS_{unit}{site}.txt"

    @classmethod
    def __init__(cls):
        cls.parse.__func__.__doc__ = cls.metricsReader.__call__.__doc__
        super().__init__()
        cls.metricsReaders = { version: cls.metricsReader(version, metricNames) for version, metricNames in cls.metricsVersions.items() }
    ...
    def parse(cls, path):
        ...
        version = cls.metricsReader.parseVersionLine(versionLine)
        try:
            return cls.metricsReaders[version](path)
        except KeyError:
    ...
    ## Nested class to allow structure of metrics files to be parameterized
    class metricsReader:
        """
